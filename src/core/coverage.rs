...
pub fn coverage_exercise_core_defensive_paths() -> Vec<bool> {
    ...
    let zero_delay_remained_active =
        state.wait_until_delay_elapsed_or_inactive(Duration::ZERO);
    ...
    let poisoned_delay_remained_active = poisoned_state
        .wait_until_delay_elapsed_or_inactive(Duration::from_millis(1));
    ...
    let removed_registration_was_not_called =
        cancellation_count.load(Ordering::SeqCst) == 0;
    ...
    let deactivate_called_registered_callback =
        cancellation_count.load(Ordering::SeqCst) == 1;
    ...
    let inactive_registration =
        inactive_state.register_delay_cancellation(move || {
            inactive_count.fetch_add(1, Ordering::SeqCst);
        });
    let inactive_registration_called_immediately = inactive_registration
        .is_none()
        && inactive_cancellation_count.load(Ordering::SeqCst) == 1;
    ...
    let poisoned_registration =
        poisoned_cancellations_state.register_delay_cancellation(|| {});
    let poisoned_cancellations_deactivate_changed_state =
        poisoned_cancellations_state.deactivate();
