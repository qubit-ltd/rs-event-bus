    ...
    T: Clone + Send + Sync + 'static,
{
    ...
    fn subscriber_id(&self) -> &str { &self.subscriber_id }

    fn priority(&self) -> i32 { self.options.priority() }

    fn deactivate(&self) { self.active.deactivate(); }
    ...
    fn dispatch(
        ...
        let event_bus = LocalEventBus { inner: Arc::clone(&bus) };
        ...
        let processing_task = ProcessingTask::with_delivery_context_and_permit(
            ...
            move || {
                ...
                process_subscription_event(
                    active, handler, options, subscription_id, subscriber_id,
                    *envelope, event_bus,
                );
            },
        );
        let result = if let Some(ordering_lane_key) = ordering_lane_key {
            if let Some(delay) = delay && !delay.is_zero() {
                bus.submit_delayed_ordered_processing_task(
                    ordering_lane_key, processing_task, delay, delayed_active, allow_stopping,
                )
            } else {
                bus.submit_ordered_processing_task(ordering_lane_key, processing_task, allow_stopping)
            }
        } else if let Some(delay) = delay && !delay.is_zero() {
