...
use crate::model::DeadLetterPolicy;
...
impl EventBusSpi for TestBackend {
    ...
    fn shutdown(&self, _: ShutdownMode) -> Result<ShutdownOutcome, SpiError> {
        ...
        self.state.lock().expect("test state lock").shutdown_calls += 1;
...
fn shutdown_from_publish_interceptor_returns_would_deadlock_instead_of_waiting_for_its_permit() {
    ...
    let (shutdown_tx, shutdown_rx) = mpsc::channel();
    let options = crate::model::PublishOptions::<String>::builder()
...
fn terminal_publish_retry_error_retains_reason_attempt_and_spi_source() {
    ...
    let options = crate::model::PublishOptions::<String>::builder()
        ...
        .retry_rule(|_: &AttemptFailure<crate::error::PublishAttemptError>, _: &RetryContext| RetryDecision::Retry)
        .build();
    ...
    let crate::error::PublishError::Retry(retry) = error else {
...
fn panicking_custom_retry_rule_requeues_instead_of_rejecting_delivery() {
    ...
    let options = SubscribeOptions::builder()
        ...
        .retry_rule(
            |_: &AttemptFailure<crate::error::DeliveryAttemptError>, _: &RetryContext| {
    ...
    let subscription = bus
        .subscribe(
            ...
            move |_| {
                ...
                Err(crate::error::DeliveryError::Handler {
...
impl qubit_event_bus::codec::EventCodec<String> for PanickingCodec {
    ...
    fn encode(&self, _: &String) -> Result<Arc<[u8]>, qubit_event_bus::error::CodecError> {
        Ok(Arc::<[u8]>::from([]))
    }
    fn decode(&self, _: &[u8]) -> Result<String, qubit_event_bus::error::CodecError> {
...
fn cancelling_subscriber_retry_terminates_its_flow_without_stopping_other_subscriptions() {
    ...
    let options = SubscribeOptions::<String>::builder()
        .ordering_policy(crate::model::OrderingPolicy::PerKey)
        .retry_policy(
            RetryPolicy::builder()
                .max_attempts(4)
                .backoff(qubit_retry::BackoffPolicy::fixed(Duration::from_secs(60)))
                .build()
                .unwrap(),
        )
        .retry_rule(|_: &AttemptFailure<crate::error::DeliveryAttemptError>, _: &RetryContext| RetryDecision::Retry)
...
fn synchronous_facade_rejects_async_subscriber_interceptors_before_provider_subscription() {
    ...
    let options = SubscribeOptions::<String>::builder()
        .async_interceptor(|_, _| {
            Box::pin(async { Ok(()) })
                as qubit_event_bus::spi::SpiFuture<'static, Result<(), crate::error::DeliveryError>>
        })
        .build();
...
fn sync_facade_rejects_async_global_subscriber_middleware() {
    let config = EventBusFacadeConfig::new().async_subscriber_interceptor(
        |_delivery: Delivery<String>, _next: crate::model::AsyncSubscriberNext<String>| {
            Box::pin(async { Ok(()) }) as crate::spi::SpiFuture<'static, Result<(), crate::error::DeliveryError>>
        },
    );
...
fn workers_canceling_each_other_do_not_form_a_join_cycle() {
    let (bus, _) = create_bus();
    let a_handle = Arc::new(Mutex::new(None::<crate::facade::Subscription>));
    let b_handle = Arc::new(Mutex::new(None::<crate::facade::Subscription>));
...
fn assert_close_failures(error: &ShutdownError, expected: &[&str]) {
    let crate::error::ShutdownError::SubscriptionClose(errors) = error else {
...
fn retry_directive_is_subject_to_qubit_retry_policy_and_abort_is_not_overridden() {
    ...
    let options = SubscribeOptions::builder()
        ...
        .error_handler(move |_, _| {
            ...
            crate::model::FailureDirective::Retry
        })
        .build();
    let subscription = bus
        .subscribe(
    ...
    let options = SubscribeOptions::builder()
        ...
        .error_handler(move |_, _| {
            callbacks_by_handler.fetch_add(1, Ordering::AcqRel);
            crate::model::FailureDirective::Discard
        })
        .build();
    let subscription = bus
        .subscribe(
...
fn dead_letter_publish_failure_requeues_instead_of_rejecting_original_message() {
    ...
    let options = SubscribeOptions::builder()
        .error_handler(|_, _| crate::model::FailureDirective::DeadLetter)
...
fn unsupported_failure_settlement_is_reported_without_calling_provider_settle() {
    ...
    let options = SubscribeOptions::builder()
        .error_handler(|_, _| crate::model::FailureDirective::Requeue)
        .build();
    let subscription = bus
        .subscribe(
...
fn unsupported_reject_settlement_is_reported_without_calling_provider_settle() {
    ...
    let options = SubscribeOptions::builder()
        .error_handler(|_, _| crate::model::FailureDirective::Discard)
        .build();
    let subscription = bus
        .subscribe(
