...
use qubit_event_bus::spi::{
    AsyncEventBusSpi, DeliveryDisposition, EventBusSpi, EventSubscriptionSpi, PayloadModes,
    ReceiveOutcome, SettlementCapabilities, SettlementToken, ShutdownMode, ShutdownOutcome,
    SpiSubscriptionRequest,
};
use support::fake_spi::{FakeAsyncEventBusSpi, FakeEventBusSpi};
...
fn sync_conformance_accepts_provider_supplied_trait_object_factory_and_gates_cases() {
    ...
    let limited_cases = run_sync_conformance(
        ...
        |_| native_no_settlement.inject_gap(),
    );
...
fn run_sync_conformance(
    ...
    inject_gap: impl Fn(&mut dyn EventSubscriptionSpi),
) -> Vec<&'static str> {
    ...
    if matches!(
        ...
        bus.publish(support::fake_spi::outbound_message()).unwrap();
        assert!(matches!(
    ...
    inject_gap(subscription.as_mut());
    assert!(matches!(
        subscription.receive(Duration::ZERO).unwrap(),
    ...
    match capabilities.settlement() {
        ...
        SettlementCapabilities::AcceptOnly | SettlementCapabilities::AcceptRetryReject => {
            ...
            assert!(
                subscription
                    .settle(
                        SettlementToken::new(subscription_id, "conformance-token"),
...
fn async_conformance_uses_manual_time_and_preserves_in_flight_message_on_cancel() {
    ...
    let limited = FakeAsyncEventBusSpi::with_capabilities(
...
fn sync_finite_timeout_rechecks_after_spurious_wake() {
    ...
    let mut subscription = bus
        .subscribe(support::fake_spi::subscription_request())
        .unwrap();
...
fn run_async_conformance(
    ...
    support::manual_async::block_on(async {
        ...
        if matches!(
            ...
            bus.publish(support::fake_spi::outbound_message())
                .await
                .unwrap();
        ...
        match capabilities.settlement() {
            ...
            SettlementCapabilities::AcceptOnly | SettlementCapabilities::AcceptRetryReject => {
                subscription
                    .settle(
                        SettlementToken::new(subscription_id, "async-token"),
                        DeliveryDisposition::Accept,
                    )
                    .await
                    .unwrap();
...
fn sync_fake_supports_injected_structured_provider_failures() {
    ...
    let error = bus
        .publish(support::fake_spi::outbound_message())
        .unwrap_err();
